...
class DocumentBlock
  extends Model<DocumentBlockAttributes>
  implements DocumentBlockAttributes
{
  declare id: number;
...
export async function verifyBlockchain(): Promise<{
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    if (i > 0) {
      ...
      if (!previousBlock) {
        errors.push(`Missing previous block for index ${i}`);
      } else if (block.previous_hash !== previousBlock.hash) {
...
export async function verifyDocumentByHash(contentHash: string): Promise<{
  ...
  if (!block) {
    return {
      valid: false,
      errors: [
        "Document not found in blockchain - this PDF was not generated by DocuChain or has been modified",
      ],
    };
  }
  ...
  if (calculatedHash !== block.hash) {
    errors.push(
      "Block hash verification failed - document may have been tampered with",
    );
  }
  ...
  if (block.previous_hash) {
    ...
    if (!previousBlock) {
      errors.push(
        "Previous block not found - blockchain integrity compromised",
      );
    }
  }
...
export async function getAnonymizedBlockchain(
  limit: number = 1000,
): Promise<AnonymizedBlock[]> {
  const blocks = await DocumentBlock.findAll({
    order: [["created_at", "ASC"]],
    limit,
  });
