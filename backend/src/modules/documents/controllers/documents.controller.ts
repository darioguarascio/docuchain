...
import { generateDocument, processMarkdownTemplate } from '@modules/documents/services/document.service.ts';
import { createBlock } from '@modules/blockchain/services/blockchain.service.ts';
import env from '@utils/env.ts';
import path from 'path';
import fs from 'fs';
import { appendHiddenMetadataBlock, buildRequestMetadata } from '@modules/documents/utils/template-metadata.ts';
import {
...
export const createDocument = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const templateWithMetadata = appendHiddenMetadataBlock(template, requestMetadata);
    ...
    if (asyncProcessing) {
      ...
      const queueName = process.env.REDIS_QUEUE || 'docuchain:documents:queue';
      await redisClient.rPush(queueName, JSON.stringify({
        documentId,
    ...
    // Generate PDF
    const pdfBuffer = await generateDocument(templateWithMetadata, placeholders || {}, outputPath);
    ...
    const processedTemplate = processMarkdownTemplate(templateWithMetadata, placeholders || {});
...
export const previewDocument = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const templateWithMetadata = appendHiddenMetadataBlock(template, requestMetadata);
    const pdfBuffer = await generateDocument(templateWithMetadata, placeholders || {});

    const { buffer: bufferWithEnvelope, encodedEnvelope } = attachPreviewEnvelope(
    ...
    res.setHeader('Content-Disposition', 'inline; filename="docuchain-preview.pdf"');
    return res.end(bufferWithEnvelope);
  } catch (error: any) {
    console.error('Error generating preview document:', error);
    return res
      .status(500)
      .json({ error: 'Failed to generate preview document', message: error.message });
  }
};
...
export const signDocument = async (req: Request, res: Response) => {
  ...
  if (!multerFile?.buffer) {
    return res.status(400).json({ error: 'PDF file upload is required (field name: pdf)' });
  }

  const pdfBuffer = multerFile.buffer;

  const { envelope: embeddedEnvelope, unsignedBuffer } = extractPreviewEnvelope(pdfBuffer);
  const providedEnvelope = decodePreviewSignatureField(req.body.preview_signature);
  const envelope = providedEnvelope ?? embeddedEnvelope;

  if (!envelope) {
    return res.status(400).json({ error: 'Preview envelope not found in upload' });
  }

  if (envelope.hmac) {
    if (!env.HMAC_SECRET_KEY) {
      return res.status(400).json({ error: 'Preview envelope is signed but server has no HMAC secret' });
    }
    const hmac = crypto
    ...
    if (hmac !== envelope.hmac) {
      return res.status(422).json({ error: 'Preview envelope HMAC verification failed' });
    }
  }
  ...
  if (actualHash !== envelope.content_hash) {
    return res.status(422).json({ error: 'Uploaded PDF does not match preview hash' });
  }
  ...
  try {
    extraMetadata = parseJsonField<Record<string, any>>(req.body.metadata, 'metadata') ?? {};
    placeholders = parseJsonField<Record<string, any>>(req.body.placeholders, 'placeholders') ?? {};
    signaturePayload = parseJsonField<any>(req.body.signature_payload, 'signature_payload') ?? [];
  } catch (error: any) {
    return res.status(400).json({ error: error.message ?? 'Invalid JSON input' });
  }
  ...
  try {
    ...
    await Document.create({
      document_id: documentId,
      template_content: envelope.metadata?.template_content ?? '[preview-sign-flow]',
      placeholders,
      status: 'completed',
      pdf_path: outputPath,
    });
    ...
    return res.status(500).json({ error: 'Failed to sign document', message: error.message });
  }
};
...
export const getDocumentPdf = async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(500).json({ error: 'Failed to fetch PDF', message: error.message });
  }
};
...
export const verifyDocument = async (req: Request, res: Response) => {
  ...
  try {
    const { verifyDocument: verifyDoc } = await import('@modules/blockchain/services/blockchain.service.ts');
    const result = await verifyDoc(documentId);

    return res.json(result);
  } catch (error: any) {
    console.error('Error verifying document:', error);
    return res.status(500).json({ error: 'Failed to verify document', message: error.message });
  }
};
...
export const verifyDocumentByFile = async (req: Request, res: Response) => {
  try {
    ...
    if (!req.body || !Buffer.isBuffer(req.body)) {
      return res.status(400).json({ error: 'PDF file is required. Send the PDF file as raw binary data with Content-Type: application/pdf' });
    }
    ...
    // Verify by hash
    const { verifyDocumentByHash } = await import('@modules/blockchain/services/blockchain.service.ts');
